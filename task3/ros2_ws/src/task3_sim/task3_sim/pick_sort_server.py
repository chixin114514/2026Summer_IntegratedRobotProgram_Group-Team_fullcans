...
class PickSortServer(Node):
    ...
    def __init__(self) -> None:
        ...
        self._action_server = ActionServer(
            self,
            SortObject,
            str(self.get_parameter("action_name").value or ACTION_NAME),
    ...
    def _transfer_verified(self, object_name: str, initial_z: float) -> bool:
        """Confirm that the object is still held after the high-level rotation."""
        pose = self._current_pose(object_name)
        rise = float("-inf") if pose is None else pose[3] - initial_z
        self._log(
            "transfer_check",
    ...
    def _execute_callback(self, goal_handle):
        ...
        try:
            ...
            for index, step in enumerate(sequence):
                ...
                if step.stage == "TRANSFER_ROTATE":
                    if not self._transfer_verified(object_name, initial_z):
                        return self._failure_result(
                            goal_handle,
                            result,
                            object_name,
                            "object dropped during loaded transfer",
                            pick_verified=pick_verified,
                        )
