...
class PoseEvidenceTests(unittest.TestCase):
    ...
    def test_settled_object_below_bin_wall_top_is_accepted(self):
        samples = [
            (99.8, -0.00564, -0.12272, 0.444),
            (99.9, -0.00564, -0.12272, 0.444),
            (100.0, -0.00564, -0.12272, 0.444),
        ]
        self.assertTrue(
            self.placement_is_valid(
                samples[-1], (-0.024314, -0.137873), samples, 100.0
            )
        )

        above_wall = [(*sample[:3], 0.446) for sample in samples]
        self.assertFalse(
            self.placement_is_valid(
                above_wall[-1], (-0.024314, -0.137873), above_wall, 100.0
            )
        )
